...
from xlr_lib import WORKROOT, add_ban, announce_ban_and_kick, announce_kick, collect_server_statuses, connect_db, create_report, fetch_platform_stats, force_random_game_tip, init_db, load_config, lookup_player, remove_ban
MANAGER_PATH = WORKROOT / 'Plutonium' / 'XLRManager.sh'

def run_manager(*args):
    if not MANAGER_PATH.exists():
        return (1, 'XLRManager not found')
    result = subprocess.run(['bash', str(MANAGER_PATH), *args], capture_output=True, text=True, timeout=120)
...
class XLRServers(commands.Cog):
    ...
    async def stats(self, ctx):
        try:
            ...
            await ctx.send(embed=xlr_embed(self.bot, description='Stats are temporarily unavailable. Try again in a few seconds.', color=XLR_DANGER))
            return
        ...
        for item in data['servers']:
            if item['online']:
                ...
                status = 'Offline'
                emoji = '🔴'
            embed.add_field(name=f"{emoji} {item['name']}", value=status, inline=False)
        await ctx.send(embed=embed)
    ...
    async def forcetip(self, ctx, server_id: str='all'):
        ...
        for item in sent:
            embed.add_field(name=item['server_id'].upper(), value=f"RCON status: **{item['players']}** player(s) parsed", inline=False)
        await ctx.send(embed=embed)
    ...
    async def report(self, ctx, target: str, *, reason: str='No reason provided'):
        ...
        report_id = create_report(conn, str(ctx.author), str(ctx.author.id), target_name, target_id, reason, 'discord', 'discord')
    ...
    async def gameban(self, ctx, target: str, *, reason: str='Banned by staff'):
        ...
        config = load_config()
        kicked = await asyncio.to_thread(announce_ban_and_kick, config, plutonium_id=plutonium_id, ip=ip, player_name=player_name, reason=reason)
    ...
    async def gamekick(self, ctx, target: str, *, reason: str='Kicked by staff'):
        ...
        player_name = rows[0]['current_name'] if rows else target
        config = load_config()
        (kicked, name) = await asyncio.to_thread(announce_kick, config, plutonium_id=plutonium_id, ip=ip, player_name=player_name, reason=reason)
        if kicked:
            ...
            embed = xlr_embed(self.bot, title='Player Not Found', description=f'`{target}` is not currently online on any XLR server.', color=XLR_DANGER)
        await ctx.send(embed=embed)
    ...
    async def dismiss(self, ctx, report_id: int):
        conn = connect_db()
        init_db(conn)
        cur = conn.execute("UPDATE reports SET status = 'dismissed' WHERE id = ? AND status = 'pending'", (report_id,))
        conn.commit()
        if cur.rowcount:
