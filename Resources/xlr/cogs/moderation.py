...
from xlr_bot_core import XLR_DANGER, add_warn, bot_owner_only, parse_duration, send_mod_log, xlr_embed, xlr_log_embed
...
class Moderation(commands.Cog):
    ...
    async def ban(self, ctx, member: discord.Member, *, reason: str='No reason provided'):
        ...
        await send_mod_log(ctx.guild, self.bot, xlr_log_embed(self.bot, 'Member Banned', f'**User:** {member} ({member.id})\n**Moderator:** {ctx.author}\n**Reason:** {reason}', color=XLR_DANGER))
    ...
    async def kick(self, ctx, member: discord.Member, *, reason: str='No reason provided'):
        ...
        await send_mod_log(ctx.guild, self.bot, xlr_log_embed(self.bot, 'Member Kicked', f'**User:** {member} ({member.id})\n**Moderator:** {ctx.author}\n**Reason:** {reason}', color=XLR_DANGER))
    ...
    async def mute(self, ctx, member: discord.Member, duration: str, *, reason: str='No reason provided'):
        ...
        await send_mod_log(ctx.guild, self.bot, xlr_log_embed(self.bot, 'Member Muted', f'**User:** {member}\n**Moderator:** {ctx.author}\n**Duration:** {duration}\n**Reason:** {reason}', color=XLR_DANGER))
    ...
    async def tempban(self, ctx, member: discord.Member, duration: str, *, reason: str='No reason provided'):
        ...
        await send_mod_log(ctx.guild, self.bot, xlr_log_embed(self.bot, 'Member Temp-Banned', f'**User:** {member}\n**Moderator:** {ctx.author}\n**Duration:** {duration}\n**Reason:** {reason}', color=XLR_DANGER))
    ...
    async def warn(self, ctx, member: discord.Member, *, reason: str='No reason provided'):
        ...
        await send_mod_log(ctx.guild, self.bot, xlr_log_embed(self.bot, 'Member Warned', f"**User:** {member}\n**Moderator:** {ctx.author}\n**Case:** #{warn['id']}\n**Reason:** {reason}", color=XLR_DANGER))
    ...
    async def sanction(self, ctx, member: discord.Member=None):
        ...
        for item in warns:
            ...
            lines.append(f"**Case #{item['id']}** · <t:{ts}:R>\nModerator: <@{item['moderator']}>\nReason: {item['reason']}")
    ...
    async def prune(self, ctx, member: discord.Member, amount: int):
        ...
        notice = await ctx.send(embed=xlr_embed(self.bot, description=f'Deleted **{len(deleted)}** messages from {member}.'))
        await asyncio.sleep(3)
        try:
            await notice.delete()
        except discord.HTTPException:
