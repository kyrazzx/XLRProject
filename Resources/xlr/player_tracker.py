...
def enabled_servers(config):
    ...
    for server in config.get('servers', []):
        if not server.get('enabled', True):
            continue
        yield {'id': server.get('id'), 'name': server.get('name', server.get('id')), 'port': server.get('port'), 'password': server.get('rcon_password') or server.get('key', ''), 'host': host, 'stdout_log': WORKROOT / 'Plutonium' / 'servers' / server.get('id') / 'logs' / 'stdout.log', 'game_log': WORKROOT / 'Plutonium' / 'storage' / 't6' / 'logs' / server.get('log_file', '')}
...
def _resolve_reporter_id(conn, reporter_name):
    ...
    if rid:
        return rid
    stripped = re.sub('^\\[[^\\]]*\\]', '', reporter_name).strip()
...
def kick_banned_player(server, client, session, reason=''):
    ...
    rcon_say(server['host'], server['port'], server['password'], ban_announce_message(name, reason))
    rcon_query(server['host'], server['port'], server['password'], f"clientkick {client['client_num']} banned")
...
def upsert_session_player(conn, session, server, entry, allow_conntrack=True):
    ...
    upsert_player(conn, plutonium_id, name, ip, entry.get('steam_id'))
...
def process_server(conn, config, server, state, offset_state):
    sid = server['id']
    session = state.setdefault(sid, {'sessions': {}, 'welcomed': set(), 'logged': set(), 'pending_ips': [], 'assigned_ips': set(), 'awaiting_connect': None})
        ...
        for line in lines:
            ...
            if steam:
                ...
                if pid and pid in session['sessions']:
                    ...
                    if entry.get('name'):
                        upsert_session_player(conn, session, server, entry, allow_conntrack=False)
    ...
    for client in parse_status_clients(status):
        ...
        if guid.isdigit():
            entry = session['sessions'].setdefault(guid, {'plutonium_id': guid, 'name': name, 'steam_id': None, 'ip': ip})
            if ip:
                entry['ip'] = ip
        if is_banned(conn, plutonium_id=plutonium_id if guid.isdigit() else None, ip=ip):
            reason = get_active_ban_reason(conn, plutonium_id=plutonium_id if guid.isdigit() else None, ip=ip)
...
def record_players_from_api(conn, config, server):
    ...
    entry = match_plutonium_server({'port': server['port'], 'name': server.get('name')}, public_ip=public_ip)
    if not entry:
        return
