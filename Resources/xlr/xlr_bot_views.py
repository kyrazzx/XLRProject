...
def safe_ticket_name(user):
    cleaned = re.sub('[^\\w-]', '', (user.name or 'user').lower())
    if cleaned:
...
class TicketPanelView(discord.ui.View):
    ...
    @discord.ui.button(label='Create Ticket', style=discord.ButtonStyle.success, custom_id='xlr_create_ticket')
    async def create_ticket(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        channel = await guild.create_text_channel(safe_ticket_name(interaction.user), category=category, overwrites=overwrites)
        embed = xlr_embed(interaction.client, title=f'Ticket for {interaction.user.display_name}', description='Please describe your issue and a staff member will assist you shortly.')
...
class CloseTicketView(discord.ui.View):
    ...
    @discord.ui.button(label='Close Ticket', style=discord.ButtonStyle.danger, custom_id='xlr_close_ticket')
