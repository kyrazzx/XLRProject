        ...
        if member.bot and await is_enabled(self.bot.store, guild.id, "antibot"):
            ...
            if not await is_whitelisted(self.bot, guild, getattr(executor, "id", None)):
                ...
                await send_mod_log(
                    ...
                    xlr_log_embed(
                        self.bot,
                        "Unauthorized Bot Removed",
                        f"**Bot:** {member} ({member.id})\n**Added by:** {executor or 'Unknown'}",
                        color=XLR_DANGER,
                    ),
                )
                return
            ...
            if count >= 6:
                ...
                await send_mod_log(
                    ...
                    xlr_log_embed(
                        self.bot,
                        "Anti-Raid",
                        f"Kicked {member} during a join flood.",
                        color=XLR_DANGER,
                    ),
                )
                return
        ...
        if welcome and welcome.get("channel"):
            ...
            if channel:
                embed = xlr_embed(
                    ...
                    description=format_welcome(welcome.get("message"), member),
                )
    ...
    async def on_member_remove(self, member):
        ...
        await send_mod_log(
            ...
            xlr_log_embed(
                self.bot,
                "Anti-Kick",
                f"**Member kicked:** {member}\n**By:** {executor} (punished)",
                color=XLR_DANGER,
            ),
        )
    ...
    async def on_guild_channel_create(self, channel):
        ...
        await send_mod_log(
            ...
            xlr_log_embed(
                self.bot,
                "Anti-Channel",
                f"Channel creation reverted.\n**By:** {executor or 'Unknown'}",
                color=XLR_DANGER,
            ),
        )
...
class SecurityRuntime:
    ...
    async def run_message_security(self, message):
        ...
        if not isinstance(member, discord.Member):
            return False
        exempt = member.guild_permissions.manage_messages or await is_whitelisted(
            self.bot, message.guild, member.id
        )
        if exempt:
            return False
        ...
        if await is_enabled(self.bot.store, message.guild.id, "antispam"):
            ...
            if count >= 6:
                ...
                try:
                    ...
                    pass
                notice = await message.channel.send(
                    embed=xlr_embed(self.bot, description=f"{member.mention} has been muted for spamming.")
                )
                await asyncio.sleep(4)
                try:
                    await notice.delete()
                ...
                await send_mod_log(
                    message.guild,
                    self.bot,
                    xlr_log_embed(self.bot, "Spam Blocked", f"**User:** {member}", color=XLR_DANGER),
                )
                return True
        return False
