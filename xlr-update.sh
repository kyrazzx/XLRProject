...
usage() {
    ...
    echo ""
    echo "  Default: git pull + merge server_config.json + repair all permissions."
...
prepare_git_update() {
    local repo_user
    repo_user=$(stat -c '%U' "$DEFAULT_DIR" 2>/dev/null || echo "")

    if [ -n "$repo_user" ] && [ "$repo_user" != "root" ]; then
        if [ "$(id -u)" -eq 0 ]; then
            chown -R "$repo_user:$repo_user" "$DEFAULT_DIR/.git" "$DEFAULT_DIR/Resources" 2>/dev/null || true
        elif command -v sudo >/dev/null 2>&1; then
            sudo chown -R "$repo_user:$repo_user" "$DEFAULT_DIR/.git" "$DEFAULT_DIR/Resources" 2>/dev/null || true
        fi
    fi
...
run_light_post_update() {
    ...
    if [ "$(id -u)" -eq 0 ]; then
        ...
        if ! sudo XLR_CONFIG_FILE="$CONFIG_FILE" bash "$DEFAULT_DIR/.config/security/setupDdosProtection.sh" --install; then
            echo "[XLR] WARN: anti-DDoS not applied — run: sudo bash $DEFAULT_DIR/.config/security/setupDdosProtection.sh --install"
        fi
    fi
    if [ -d "$DEFAULT_DIR/Resources/binaries/zone" ]; then
        source "$DEFAULT_DIR/.config/binaries/gameFiles.sh" --import
        ensureZoneLocaleAliases "$DEFAULT_DIR/Resources/binaries/zone" || true
    fi
}
...
source "$DEFAULT_DIR/.config/xlr/normalizeLineEndings.sh" --import
xlr_normalize_line_endings "$DEFAULT_DIR" || true
...
if [ -f "$BACKUP_DIR/secrets.env" ]; then
    ...
    sudo chmod 640 "$SECRETS_FILE" 2>/dev/null || true
fi

source "$DEFAULT_DIR/.config/xlr/repairPermissions.sh"
xlr_repair_permissions "$DEFAULT_DIR"
...
echo "  bash $DEFAULT_DIR/.config/xlr/setupResxtScripts.sh --install"
echo ""
