...
xlr_chat_commands_cn="是否安装Resxt聊天命令（仅owner，XLR品牌）（Y/n）？"
xlr_mapvote_cn="是否安装Resxt地图投票（比赛结束时）（Y/n）？"
manual_game_files_cn="手动导入游戏文件（不使用torrent）（Y/n）？"
manual_upload_help_cn="请将游戏文件放到以下路径后重新运行安装或 import-game-files.sh："
